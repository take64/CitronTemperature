...
kern_return_t SMCCall(int index, SMCKeyData_t *inputStructure, SMCKeyData_t *outputStructure)
{
    ...
    inputStructCnt = sizeof(SMCKeyData_t);
    outputStructCnt = sizeof(SMCKeyData_t);

    return IOConnectCallStructMethod(
               conn,
               index,
               inputStructure, inputStructCnt,
               outputStructure, &outputStructCnt
             );
}
    ...
    while ((c = getopt(argc, argv, "fhk:lrw:v")) != -1)
    {
        switch(c)
        {
                ...
                op = OP_WRITE;
            {
                ...
                {
                    sprintf(c, "%c%c", optarg[i], optarg[i + 1]);
                    val.bytes[i / 2] = (int) strtol(c, NULL, 16);
                }
                val.dataSize = i / 2;
